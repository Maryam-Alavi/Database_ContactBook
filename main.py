...
cursor.execute(
    "CREATE TABLE IF NOT EXISTS CONTACTBOOK (Id INTEGER PRIMARY KEY, NAME TEXT, ADDRESS TEXT, EMAILADDRESS TEXT, PHONENUMBER INTEGER )")
connector.commit()
...
def menu():
    while (user_input := input(MENU_PROMPT)) != "5":
        if user_input == "1":
            ...
            connector = sqlite3.connect('data.db')
            cursor = connector.cursor()
            cursor.execute("INSERT INTO CONTACTBOOK (Name, Address, EmailAddress, Phonenumber) VALUES (?, ?, ?, ?)",
                           (Name, Address, EmailAddress, PhoneNumber))
            connector.commit()
            ...
            cursor.execute(
                "Update CONTACTBOOK SET Name = ? , Address = ?, EmailAddress = ?, PhoneNumber = ? WHERE ID= ?  ",
                [Name, Address, EmailAddress, PhoneNumber, Id])
